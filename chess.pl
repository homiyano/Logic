...
move_result(Pos, move(Color, From, To, Type), NewPos) :-
    ...
    ( select(piece(OC, OT, To), PosWithoutFrom, PosWithoutTo) -> true ; PosWithoutTo = PosWithoutFrom ),
...
% in_check(+Pos, +Color) - true if Color's king is attacked by any opponent piece
in_check(Pos, Color) :-
...
illegal(move(Color, From, To, Type), Reason) :-
